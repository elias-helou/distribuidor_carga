...
import { Disciplina, Docente, horariosSobrepoem, TipoTrava } from "../Global/utils";
...
export interface Cell { // Representa cada célula da tabela
...
export function ProcessWrapper({ children }: { children: React.ReactNode }) {
    const {disciplinas: globalDisciplinas, docentes: globalDocentes} = useGlobalContext()
    const [disciplinas, setDisciplinas] = useState<Map<string, Disciplina>>(new Map<string, Disciplina>());
    ...
    useEffect(() => {
        ...
        if(globalDisciplinas.length > 0) {
            ...
            const newDiciplinas: Map<string, Disciplina> = new Map<string, Disciplina>();
            ...
            for(let i = 0; i < globalDisciplinas.length; i++) {
                ...
                if(!newDiciplinas.has(globalDisciplinas[i].id)) {
                    ...
                    newDiciplinas.set(globalDisciplinas[i].id, {...globalDisciplinas[i], conflitos: new Set<string>()})
                }

                const disciplinaPivo: Disciplina = newDiciplinas.get(globalDisciplinas[i].id)
                ...
                for(let j = i + 1; j < globalDisciplinas.length; j++) {
                    ...
                    if(!newDiciplinas.has(globalDisciplinas[j].id)) {
                        ...
                        newDiciplinas.set(globalDisciplinas[j].id, {...globalDisciplinas[j], conflitos: new Set<string>()})
                    }

                    const disciplinaAtual: Disciplina = newDiciplinas.get(globalDisciplinas[j].id)
    ...
    if(globalDocentes.length > 0) {
        ...
        for(const docente of globalDocentes) {
            ...
            if(disciplinas.size > 0) {
                ...
                newDocente = {...docente, formularios: formulariosDisciplinasAtivas}
            }else {
                newDocente = {...docente}
            }
  ...
  return (
    <ProcessContext.Provider value={{disciplinas, setDisciplinas, docentes, setDocentes}}>
      {children}
